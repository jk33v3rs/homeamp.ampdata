...
# Target: Hetzner Xeon (135.181.212.169) - archivesmp.site
# Purpose: Complete wipe and fresh installation from latest GitHub commit
# Access: Run via NoMachine terminal on Hetzner server
...
echo "Target: Hetzner (archivesmp.site)"
echo "========================================"
echo ""
...
echo ""
echo "[7/11] Initializing database schema..."
mysql -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" < "$INSTALL_DIR/src/database/schema.sql"
...
echo ""
echo "Next steps:"
echo "  1. Check web UI: http://135.181.212.169:8000/static/index.html"
echo "  2. Check agent API: http://135.181.212.169:8001/api/agent/status"
