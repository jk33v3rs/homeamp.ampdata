...
class PluginUpdateInfrastructure:
    ...
    def _create_staging_bucket(self) -> aws.s3.Bucket:
        ...
        bucket = aws.s3.Bucket(
            ...
            acl="private",
            versioning=aws.s3.BucketVersioningArgs(enabled=True),  # Keep version history
            lifecycle_rules=[
            ...
            tags={"Project": self.project_name, "Environment": self.environment, "Purpose": "plugin-staging"},
        )
    ...
    def _create_excel_bucket(self) -> aws.s3.Bucket:
        ...
        bucket = aws.s3.Bucket(
            ...
            tags={"Project": self.project_name, "Environment": self.environment, "Purpose": "excel-reports"},
        )
    ...
    def _create_lambda_role(self) -> aws.iam.Role:
        ...
        s3_policy = aws.iam.RolePolicy(
            ...
            role=role.name,
            policy=Output.all(self.staging_bucket.arn, self.excel_bucket.arn).apply(
                lambda args: f"""{{
                "Version": "2012-10-17",
                "Statement": [{{
                    "Effect": "Allow",
    ...
    def _create_lambda_function(self) -> aws.lambda_.Function:
        ...
        lambda_code = pulumi.AssetArchive({".": pulumi.FileArchive("./lambda_package.zip")})  # Pre-built package
        ...
        function = aws.lambda_.Function(
            ...
            tags={"Project": self.project_name, "Environment": self.environment},
        )
    ...
    def _create_schedule_rule(self) -> aws.cloudwatch.EventRule:
        ...
        target = aws.cloudwatch.EventTarget(
            f"{self.project_name}-lambda-target", rule=rule.name, arn=self.lambda_function.arn
        )
    ...
    def _configure_notifications(self):
        ...
        aws.iam.RolePolicy(
            ...
            policy=self.sns_topic.arn.apply(
                lambda arn: f"""{{
                "Version": "2012-10-17",
                "Statement": [{{
                    "Effect": "Allow",
