...
class AutomatedPluginManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.amp_client = AMPClient(
            base_url=config["amp_url"], username=config["amp_username"], password=config["amp_password"]
        )
        ...
        self.excel_reader = ExcelConfigReader(
            Path(config.get("reference_data_dir", "/var/lib/archivesmp/data/reference_data"))
        )
        ...
        for update in updates:
            try:
                ...
                success = self.amp_deployer.deploy_plugin(
                    instance_id=self.dev_instance_id, plugin_jar=plugin_jar, backup_old=True, restart_server=True
                )
    ...
    def manual_deploy(self, plugin_name: str, version: str, instance_id: str) -> bool:
        ...
        try:
            ...
            return self.amp_deployer.deploy_plugin(
                instance_id=instance_id, plugin_jar=plugin_jar, backup_old=True, restart_server=True
            )
...
async def main():
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler("/var/log/archivesmp/plugin-automation.log"), logging.StreamHandler()],
    )
