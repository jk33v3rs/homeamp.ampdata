...
class ConfigHierarchyResolver:
    ...
    def _query_matching_rules(self, context: ConfigContext) -> List[Dict[str, Any]]:
        ...
        conditions = ["plugin_id = %s", "config_file = %s", "config_key = %s", "is_active = TRUE"]
        ...
        if context.world_name and context.instance_id:
            scope_conditions.append("(scope_level = 'WORLD' AND instance_id = %s AND world_name = %s)")
            params.extend([context.instance_id, context.world_name])

        # RANK
        if context.rank_name and context.instance_id:
            scope_conditions.append("(scope_level = 'RANK' AND instance_id = %s AND rank_name = %s)")
            params.extend([context.instance_id, context.rank_name])

        # PLAYER
        if context.player_uuid and context.instance_id:
            scope_conditions.append("(scope_level = 'PLAYER' AND instance_id = %s AND player_uuid = %s)")
    ...
    def _select_best_rule(self, rules: List[Dict[str, Any]], context: ConfigContext) -> Optional[Dict[str, Any]]:
        """
        ...
        if len(scored_rules) > 1:
            logger.debug(f"Selected rule at {best_rule['scope_level']} scope " f"over {len(scored_rules) - 1} other(s)")

        return best_rule

    def _calculate_scope_score(self, rule: Dict[str, Any], context: ConfigContext) -> int:
        """
    ...
    def resolve_all_configs(
        self, context: ConfigContext, config_keys: Optional[List[str]] = None
        ...
        if config_keys:
            ...
            available_keys = self._get_available_keys(context.plugin_id, context.config_file)
