...
class ProductionDataLoader:
    ...
    def _discover_servers(self) -> Dict[str, ServerInfo]:
        ...
        for platform in ["OVH", "Hetzner"]:
            ...
            for server_dir in platform_path.iterdir():
                ...
                server_config = (
                    self.server_configs.get("archivesmp_server_configurations", {})
                    .get("servers", {})
                    .get(server_id, {})
                )
    ...
    def load_universal_plugin_configs(self) -> Dict[str, Dict[str, Any]]:
        ...
        universal_configs_dir = (
            self.base_path / "software" / "homeamp-config-manager" / "data" / "baselines" / "universal_configs"
        )
