...
PUBLIC_INSTANCES = {
    ...
    'EVO01', 'HARD01', 'MINE01', 'MIN01', 'PRI01', 'SMP101', 'SMP201', 'TOW01'
}
...
def main():
    ...
    try:
        ...
        public_config = generator.save_liveatlas_config(
            'public',
            Path('/tmp/liveatlas-public-config.json')
        )
        public_nginx = generator.save_nginx_config(
            'public',
            Path('/tmp/nginx-public-pl3xmap.conf')
        )
        
        # Generate BTS map config
        bts_config = generator.save_liveatlas_config(
            'bts',
            Path('/tmp/liveatlas-bts-config.json')
        )
        bts_nginx = generator.save_nginx_config(
            'bts',
