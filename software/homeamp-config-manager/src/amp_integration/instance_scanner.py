...
class AMPInstanceScanner:
    ...
    def _scan_minecraft_instance(self, instance_dir: Path, minecraft_dir: Path) -> Dict[str, Any]:
        ...
        return {
            ...
            "paper_yml": (
                str(minecraft_dir / "config" / "paper-global.yml")
                if (minecraft_dir / "config" / "paper-global.yml").exists()
                else None
            ),
            "pufferfish_yml": (
                str(minecraft_dir / "pufferfish.yml") if (minecraft_dir / "pufferfish.yml").exists() else None
            ),
