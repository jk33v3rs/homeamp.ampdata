...
app = FastAPI(
    title="ArchiveSMP Agent API", description="REST API for config deployment and instance management", version="1.0.0"
)
    ...
    for instance_name, plugins_config in request.configs.items():
        try:
            ...
            if not instance_path.exists():
                results[instance_name] = {"success": False, "error": f"Instance directory not found: {instance_path}"}
                continue
            ...
            for plugin_name, config_data in plugins_config.items():
                try:
                    plugin_result = _deploy_plugin_config(instance_path, plugin_name, config_data)
                    plugin_results[plugin_name] = plugin_result

                except Exception as e:
                    plugin_results[plugin_name] = {"success": False, "error": str(e)}

            # Check if all plugins deployed successfully
            all_success = all(r.get("success", False) for r in plugin_results.values())

            results[instance_name] = {"success": all_success, "plugins": plugin_results}
...
async def mark_restart_needed(instance_name: str) -> Dict[str, Any]:
    ...
    needs_restart_instances.add(instance_name)
    return {"success": True, "instance": instance_name, "needs_restart": list(needs_restart_instances)}
...
def _deploy_plugin_config(instance_path: Path, plugin_name: str, config_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "plugin": plugin_name, "instance": instance_path.name, "error": str(e)}
...
def _execute_amp_command(command: str, instance: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "command": command, "error": "Command timed out after 5 minutes"}
    except Exception as e:
...
async def start_tile_sync():
    ...
    try:
        result = subprocess.run(
            ["sudo", "systemctl", "start", "pl3xmap-tile-sync"], capture_output=True, text=True, timeout=10
        )
...
async def stop_tile_sync():
    ...
    try:
        result = subprocess.run(
            ["sudo", "systemctl", "stop", "pl3xmap-tile-sync"], capture_output=True, text=True, timeout=10
        )
...
async def get_tile_sync_status():
    ...
    try:
        ...
        result = subprocess.run(
            ["systemctl", "is-active", "pl3xmap-tile-sync"], capture_output=True, text=True, timeout=5
        )
        ...
        return {"status": status, "instances": instances, "service": "pl3xmap-tile-sync"}
