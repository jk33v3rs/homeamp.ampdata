...
class AgentDatabaseMethods:
    ...
    def _load_plugin_registry(self):
        ...
        try:
            # Query all plugins
            cursor = self.db.execute_query("SELECT * FROM plugins", fetch=True)
    ...
    def _load_datapack_registry(self):
        ...
        try:
            cursor = self.db.execute_query("SELECT * FROM datapacks", fetch=True)
    ...
    def _register_plugin_installation(self, instance_id: str, plugin_id: str, plugin_info: Dict, jar_path: Path):
    ...
    def _register_datapack_installation(
        self, instance_id: str, datapack_id: str, world_name: str, datapack_info: Dict, datapack_path: Path
    ):
        ...
        try:
            ...
            self.db.execute_query(
                ...
                ON DUPLICATE KEY UPDATE
                    ...
                    last_scanned_at = NOW()
            """,
                {**common_props, "instance_id": instance_id, "properties_json": json.dumps(properties)},
            )
    ...
    def _start_discovery_run(self, run_type: str) -> int:
        ...
        """,
            {"server_name": self.server_name, "run_type": run_type},
            fetch=True,
        )
    ...
    def _end_discovery_run(self, status: str, stats: Dict = None):
        ...
        update_data = {"run_id": self.current_run_id, "status": status}
    ...
    def _auto_tag_instance(self, instance_id: str):
        ...
        try:
            ...
            if "vault" in installed_plugins and any(
                p in installed_plugins for p in ["essentialsx", "economy", "shopkeeper"]
            ):
        ...
        try:
            ...
            self.db.execute_query(
                ...
                ON DUPLICATE KEY UPDATE
                    ...
                    applied_at = NOW()
            """,
                {"instance_id": instance_id, "tag_id": tag_id, "confidence": confidence},
            )
