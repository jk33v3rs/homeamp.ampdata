...
class ConflictDetector:
    ...
    def check_concurrent_changes(
        ...
        if len(changes) > 1:
            logger.warning(
                f"Detected {len(changes)} concurrent changes to "
                f"{plugin_name}/{config_key} on {instance_id}"
            )
    ...
    def lock_config(
        ...
        if existing_lock:
            if existing_lock['locked_by'] == locked_by:
                ...
                logger.warning(
                    f"Config locked by {existing_lock['locked_by']} "
                    f"until {existing_lock['locked_until']}"
                )
                return False
