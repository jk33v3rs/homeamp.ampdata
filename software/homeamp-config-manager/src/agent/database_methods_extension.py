...
def _register_endpoint_config_file(
    self,
    ...
    try:
        if config_file_id:
            ...
            """,
                {"config_file_id": config_file_id, "current_hash": current_hash, "file_size": file_size},
            )
...
def _create_config_backup(
    self, config_file_id: int, instance_id: int, file_path: str, reason: str = "manual"
) -> Optional[int]:
    """
...
def _mark_config_outdated(self, config_file_id: int, old_hash: str, new_hash: str) -> bool:
    ...
    try:
        ...
        self.db.execute_query(
            ...
            WHERE config_file_id = %(config_file_id)s
        """,
            {"config_file_id": config_file_id, "new_hash": new_hash},
        )
...
def _list_backups(self, config_file_id: int, limit: int = 10) -> List[Dict]:
    ...
    try:
        result = self.db.execute_query(
            ...
            LIMIT %(limit)s
        """,
            {"config_file_id": config_file_id, "limit": limit},
            fetch=True,
        )
