...
class UpdateChecker:
    ...
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": "ArchiveSMP-ConfigManager/2.0"})
            ...
            for plugin in plugins:
                try:
                    latest_version = self.check_plugin_update(
                        plugin["source_type"], plugin["source_url"], plugin["source_identifier"]
                    )

                    if latest_version:
                        # Compare versions
                        if self.is_newer_version(plugin["current_version"], latest_version):
                            self.record_update_available(plugin["plugin_id"], plugin["current_version"], latest_version)
                            updates_found += 1
                            logger.info(
                                f"Update found for {plugin['name']}: {plugin['current_version']} → {latest_version}"
                            )
                        else:
                            logger.debug(
                                f"No update for {plugin['name']} (current: {plugin['current_version']}, latest: {latest_version})"
                            )
