...
class VarianceDetector:
    ...
    def scan_instance_configs(
        self, instance_id: str, instance_path: str, plugin_list: List[str]
    ) -> List[ConfigVariance]:
            ...
            for file_path, baseline_keys in baseline.items():
                ...
                for config_key, baseline_value in baseline_keys.items():
                    ...
                    if actual_value != baseline_value:
                        ...
                        logger.info(
                            f"Variance detected: {instance_id}/{plugin_name}/{config_key}: {actual_value} vs {baseline_value}"
                        )

        return variances

    def register_variance(
        self, instance_id: str, plugin_name: str, config_key: str, baseline_value: Any, actual_value: Any
    ) -> int:
