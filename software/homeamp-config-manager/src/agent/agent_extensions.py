...
def _map_folder_to_instance_id(self, folder_name: str, amp_uuid: Optional[str]) -> Optional[int]:
    ...
    if amp_uuid:
        result = self.db.execute_query("SELECT instance_id FROM instances WHERE amp_instance_id = %s", (amp_uuid,))
        if result:
            return result[0]["instance_id"]

    # Fall back to matching by instance_name
    result = self.db.execute_query("SELECT instance_id FROM instances WHERE instance_name = %s", (folder_name,))
    if result:
        return result[0]["instance_id"]
...
def _update_instance_paths(self, instance_id: int, folder_name: str, base_path: str, amp_uuid: Optional[str] = None):
    ...
    """
    update_fields = {"instance_folder_name": folder_name, "instance_base_path": base_path}
...
def _scan_all_plugin_configs(self):
    ...
    for instance_info in instances:
        ...
        self._update_instance_paths(instance_id, folder_name, instance_info["base_path"], instance_info["amp_uuid"])
