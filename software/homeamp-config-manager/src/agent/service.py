...
from ..core.cloud_storage import ChangeRequestManager, CloudStorage
...
class AgentService:
    ...
    def __init__(self, config_file: Optional[Path] = None):
        ...
        if self.settings.log_to_file:
            log_file = self.settings.get(
                "agent", "logging", "log_file", default=str(self.settings.data_dir / "agent.log")
            )
        ...
        logging.basicConfig(level=log_level, format=self.settings.log_format, handlers=handlers)
    ...
    def _load_config(self, config_file: Path) -> Dict[str, Any]:
        ...
        try:
            if not config_file.exists():
                ...
                default_config = {
                    "server": {"name": "unknown", "path": "/opt/yunohost/minecraft", "type": "paper"},
                    "minio": {
    ...
    def _do_process_changes(self):
        ...
        try:
            for change_request in pending_changes:
                ...
                if target_servers != "all" and self.server_name not in target_servers:
                    continue
    ...
    def _apply_change_request(self, change_id: str, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result.update(
                {"server_name": self.server_name, "applied_at": datetime.now().isoformat(), "agent_version": "1.0.0"}
            )
