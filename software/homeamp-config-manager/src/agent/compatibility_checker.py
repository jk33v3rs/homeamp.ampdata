...
class CompatibilityChecker:
    ...
    def check_version_compatibility(self, plugin_name: str, from_version: str, to_version: str) -> Dict[str, Any]:
        """
        ...
        self.cursor.execute(
            query,
            (plugin_name, from_version, f"{from_version.split('.')[0]}.%", to_version, f"{to_version.split('.')[0]}.%"),
        )
        ...
        if breaking_migrations:
            ...
            result["breaking_changes"] = [
                {"key": m["old_key_path"], "new_key": m["new_key_path"], "note": m["notes"]}
        ...
        if to_major > from_major:
            result["warnings"].append(f"Major version upgrade ({from_major} → {to_major}) - review changelog carefully")
            result["recommendations"].append("Test on staging instance before deploying to production")
    ...
    def check_minecraft_compatibility(
        self, plugin_name: str, plugin_version: str, minecraft_version: str
    ) -> Dict[str, Any]:
        """
        ...
        if not metadata:
            return {"compatible": "unknown", "warning": "No compatibility metadata available"}
    ...
    def check_plugin_dependencies(self, plugin_name: str, installed_plugins: List[Dict[str, str]]) -> Dict[str, Any]:
        """
        ...
        for req_plugin, req_version in required.items():
            if req_plugin not in installed_map:
                ...
                if installed_ver < required_ver:
                    incompatible.append(
                        {"plugin": req_plugin, "installed": installed_map[req_plugin], "required": req_version}
                    )
    ...
    def validate_upgrade_path(
        self, plugin_name: str, from_version: str, to_version: str, instance_id: str
    ) -> Dict[str, Any]:
        """
        ...
        """
        report = {"can_upgrade": True, "blockers": [], "warnings": [], "steps_required": []}
        ...
        if not compat["compatible"]:
            ...
            report["blockers"].extend(
                [f"Breaking change: {bc['key']} → {bc['new_key']}" for bc in compat["breaking_changes"]]
            )
            report["steps_required"].append(f"Apply {len(compat['migrations_required'])} config migrations")
        ...
        if instance:
            mc_compat = self.check_minecraft_compatibility(plugin_name, to_version, instance["minecraft_version"])
