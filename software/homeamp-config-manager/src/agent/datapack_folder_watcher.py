...
class DatapackChangeHandler(FileSystemEventHandler):
    ...
    def force_scan(self):
        ...
        if self.pending_changes:
            logger.info(
                f"[{self.instance_name}/{self.world_name}] Forcing immediate scan for {len(self.pending_changes)} changes"
            )
            self.scan_callback(self.instance_id, self.instance_name, self.world_name, list(self.pending_changes))
            self.pending_changes.clear()
            self.last_scan = time.time()
...
class DatapackFolderWatcher:
    ...
    def __init__(self, amp_data_path: str = "/home/amp/.ampdata"):
        ...
        self.watched_worlds: Dict[str, tuple] = (
            {}
        )  # "instance_name:world_name" -> (instance_id, instance_name, world_name)
    ...
    def get_watched_worlds(self) -> List[Dict]:
        ...
        return [
            {"instance_name": instance_name, "instance_id": instance_id, "world_name": world_name}
