...
async def get_instance(instance_id: str):
    ...
    return {**instance, "groups": groups, "tags": tags}
...
async def resolve_config(instance_id: str, plugin: str, config_file: str, key: str):
    """
    ...
    """
    try:
        value, priority, scope = db.resolve_config_value(instance_id, plugin, config_file, key)
...
async def get_change_statistics():
    ...
    try:
        ...
        return {"total_changes": total, "by_type": by_type, "by_user": by_user, "last_7_days": recent}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
...
async def list_all_migrations():
    ...
    try:
        ...
        return {"plugins": summary, "total_plugins": len(summary)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
...
async def get_plugin_migrations(plugin_name: str):
    ...
    try:
        ...
        if not migrations:
            ...
            return {"plugin": plugin_name, "migrations": [], "total": 0}
    ...
    try:
        ...
        for rule in rules:
            ...
            if scope == "universal" and rule["scope_type"] == "GLOBAL":
                ...
                if config_structure[full_key]["value"] is None or rule["priority"] < config_structure[full_key].get(
                    ...
                    if rule["scope_selector"]:
                        config_structure[full_key][
                            "source_description"
                        ] = f"Override: {rule['scope_type']} = {rule['scope_selector']}"
        ...
        for plugin_id, config_changes in changes.items():
            for config_key, value in config_changes.items():
                try:
                    ...
                    errors.append({"plugin_id": plugin_id, "config_key": config_key, "error": str(e)})

        return {"success": len(errors) == 0, "total_changes": len(results), "successful": results, "errors": errors}
...
async def get_variance_summary():
    ...
    summary = {"by_classification": {}, "total_configs": 0, "total_drift": 0}

    for row in db.cursor.fetchall():
        classification = row["variance_classification"]
        summary["by_classification"][classification] = {"total": row["count"], "drift": row["drift_count"]}
...
async def get_plugin_variance(plugin_name: str):
    ...
    return {"plugin_name": plugin_name, "total_configs": len(variances), "variances": variances}
...
async def resolve_config_rule(instance_id: str, plugin_name: str, config_key: str):
    """
...
async def create_config_rule(rule: Dict[str, Any]):
    ...
    scope_priorities = {"INSTANCE": 1, "META_TAG": 2, "SERVER": 3, "GLOBAL": 4}
    ...
    return {"success": True, "rule_id": db.cursor.lastrowid}
...
async def get_all_tags():
    ...
    return {"all_tags": tags, "by_category": by_category}
...
async def create_tag_category(category: Dict[str, Any]):
    ...
    db.cursor.execute(
        ...
        VALUES (%s, %s, %s)
    """,
        (category["category_name"], category.get("description", ""), category.get("display_order", next_order)),
    )
    db.commit()
...
async def get_deployment_history(
    deployed_by: str = None, status: str = None, deployment_type: str = None, limit: int = 50, offset: int = 0
):
...
async def get_plugin_migrations(plugin_name: str):
    ...
    return {"plugin": plugin_name, "migrations": migrations, "total": len(migrations)}


@app.get("/api/variance/history")
async def get_variance_history(
    instance_id: str = None, plugin_name: str = None, config_key: str = None, limit: int = 100
):
...
async def get_notification_log(notification_type: str = None, status: str = None, limit: int = 50):
