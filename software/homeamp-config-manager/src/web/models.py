...
class DeviationParser:
            ...
            try:
                ...
                for plugin_name, plugin_configs in production_configs.items():
                    configs[plugin_name] = PluginConfig(
                        plugin=plugin_name, config_file="production_configs", configs=plugin_configs
                    )
    ...
    def _save_universal_config(
        self, configs: Dict[str, PluginConfig], plugin: str, config_file: str, content: List[str]
    ):
        ...
        try:
            ...
            if yaml_content.strip():
                ...
                if plugin not in configs:
                    configs[plugin] = PluginConfig(plugin=plugin, config_file=config_file, configs={})
    ...
    def generate_fix_changes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not flagged_deviations:
                return {"success": False, "message": "No flagged deviations to fix", "changes": []}
            ...
            return {"success": False, "error": str(e), "message": "Failed to generate fix changes"}
