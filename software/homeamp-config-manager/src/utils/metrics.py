...
class MetricsExporter:
    ...
    def __init__(self):
        ...
        self.agent_up = Gauge(
            "homeamp_agent_up", "Agent is running (1) or down (0)", ["server"], registry=self.registry
        )

        self.last_poll_timestamp = Gauge(
            "homeamp_agent_last_poll_timestamp", "Unix timestamp of last MinIO poll", ["server"], registry=self.registry
        )
        ...
        self.deviations_pending = Gauge(
            "homeamp_deviations_pending", "Number of deviations pending review", ["server"], registry=self.registry
        )

        self.deviations_flagged_bad = Gauge(
            "homeamp_deviations_flagged_bad", "Number of deviations flagged as bad", ["server"], registry=self.registry
        )
