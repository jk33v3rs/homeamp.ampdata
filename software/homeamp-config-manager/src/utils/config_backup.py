...
class ConfigBackup:
    ...
    def create_backup(
        ...
        try:
            ...
            cursor.execute(
                "SELECT plugin_id FROM endpoint_config_files WHERE id = %s",
                (config_file_id,)
            )
            ...
            """, (
                config_file_id, instance_id, plugin_id, str(file_path),
                content, file_hash, backed_up_by, reason, metadata_json
            ))
