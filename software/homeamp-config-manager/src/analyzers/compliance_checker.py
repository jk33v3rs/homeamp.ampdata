...
class ComplianceChecker:
    ...
    def check_expected_changes(self, expected_changes_path: Path) -> Dict[str, Any]:
        ...
        try:
                ...
                try:
                    ...
                    if not all([plugin_name, config_file, key_path, expected_value is not None]):
                        pending_changes.append(
                            {"change": change, "status": "invalid", "reason": "Missing required fields"}
                        )
                        continue
                    ...
                    if server_name == "all":
                            ...
                            if config_data:
                                ...
                                if current_value == expected_value:
                                    completion_details.append(
                                        {"server": srv_name, "status": "completed", "current_value": current_value}
                                    )
                        ...
                        if config_data:
                            ...
                            if current_value == expected_value:
                                change_completed = True
                                completion_details.append(
                                    {"server": server_name, "status": "completed", "current_value": current_value}
                                )
                            else:
                                completion_details.append(
                                    {
                            ...
                            completion_details.append(
                                {"server": server_name, "status": "pending", "reason": "Configuration not found"}
                            )
                    ...
                    pending_changes.append({"change": change, "status": "error", "reason": str(e)})
            ...
            return {"completed_changes": [], "pending_changes": [], "summary": {"error": str(e)}}
    ...
    def _generate_next_actions(self, comparison_results: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        try:
                ...
                actions.append(
                    {
                        ...
                        "target": (
                            item.get("servers_missing", ["unknown"])[0] if item.get("servers_missing") else "unknown"
                        ),
                    }
                )
            ...
            actions.append({"action": "error", "priority": "high", "description": f"Error generating actions: {e}"})
