        ...
        try:
            ...
            for plugin_name, baseline_plugin in baseline.items():
                ...
                for config_file, baseline_config in baseline_plugin.items():
                    ...
                    drift_items.extend(
                        self._compare_configs(plugin_name, config_file, baseline_config, current_config, server_name)
                    )
                    ...
                    if isinstance(baseline_value, dict) and isinstance(current_value, dict):
                        drift_items.extend(
                            self._compare_configs(
                                plugin_name, config_file, baseline_value, current_value, server_name, key_path
                            )
                        )
                    # List comparison
