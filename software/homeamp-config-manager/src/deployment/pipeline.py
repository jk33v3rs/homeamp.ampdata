...
class DeploymentPipeline:
    ...
    def deploy_to_dev(self, config_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "deployment_id": deployment_id, "error": str(e), "environment": "dev"}
    ...
    def validate_dev_deployment(self, deployment_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_deployments = [server for server, result in dev_results.items() if result.get("success")]
            failed_deployments = [server for server, result in dev_results.items() if not result.get("success")]
            ...
            validation_results["checks"].append(
                {
                    ...
                    "details": {"successful_servers": successful_deployments, "failed_servers": failed_deployments},
                }
            )
            ...
            for server_name in successful_deployments:
                try:
                    ...
                    if server_path.exists():
                        ...
                        config_details[server_name] = {"config_files_found": len(config_files), "status": "valid"}
                    else:
                        config_details[server_name] = {"status": "server_path_not_found"}
                        config_check_passed = False
                except Exception as e:
                    config_details[server_name] = {"status": "error", "error": str(e)}
                    config_check_passed = False
    ...
    def deploy_to_production(self, deployment_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "deployment_id": deployment_id, "error": str(e), "environment": "production"}
            ...
            for server_name in affected_servers:
                try:
                    ...
                    if "results" in deployment and server_name in deployment["results"]:
                        ...
                        "prod_deployment" in deployment
                        and "results" in deployment["prod_deployment"]
                        and server_name in deployment["prod_deployment"]["results"]
                    ):
