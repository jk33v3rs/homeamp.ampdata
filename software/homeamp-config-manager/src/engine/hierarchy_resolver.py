...
class HierarchyResolver:
    ...
    def resolve_config_value(
        ...
        if global_value is not None:
            ...
            source_scope = "GLOBAL"
            resolution_chain.append({"scope": "GLOBAL", "value": global_value, "overridden": False})
            ...
            if server_name:
                ...
                if server_value is not None:
                    ...
                    resolution_chain.append(
                        {"scope": "SERVER", "server": server_name, "value": server_value, "overridden": False}
                    )
        ...
        if instance_id:
            ...
            for tag in meta_tags:
                ...
                if tag_value is not None:
                    ...
                    resolution_chain.append(
                        {"scope": "META_TAG", "tag": tag["tag_name"], "value": tag_value, "overridden": False}
                    )
        ...
        if instance_id:
            ...
            if instance_value is not None:
                ...
                resolution_chain.append(
                    {"scope": "INSTANCE", "instance_id": instance_id, "value": instance_value, "overridden": False}
                )
        ...
        if instance_id and world_name:
            ...
            if world_value is not None:
                ...
                source_scope = "WORLD"
                resolution_chain.append(
                    {"scope": "WORLD", "world": world_name, "value": world_value, "overridden": False}
                )
        ...
        if instance_id and rank_name:
            ...
            if rank_value is not None:
                ...
                source_scope = "RANK"
                resolution_chain.append({"scope": "RANK", "rank": rank_name, "value": rank_value, "overridden": False})
        ...
        if instance_id and player_uuid:
            ...
            if player_value is not None:
                ...
                source_scope = "PLAYER"
                resolution_chain.append(
                    {"scope": "PLAYER", "player_uuid": player_uuid, "value": player_value, "overridden": False}
                )

        # Apply variable substitution
        if isinstance(resolved_value, str):
            resolved_value = self.apply_substitution_vars(
                resolved_value, instance_id, world_name, rank_name, player_uuid
            )
    ...
    def get_effective_value(
        ...
        """
        value, _, _ = self.resolve_config_value(plugin_id, config_key, instance_id, world_name, rank_name, player_uuid)
        return value
    ...
    def get_override_chain(
        ...
        """
        _, _, chain = self.resolve_config_value(plugin_id, config_key, instance_id, world_name, rank_name, player_uuid)
        return chain

    def apply_substitution_vars(
        self,
        value: str,
        instance_id: int = None,
        world_name: str = None,
        ...
        if instance_id:
            instance_info = self.db.execute_query(
                "SELECT instance_name, server_name FROM instances WHERE instance_id = %s", (instance_id,), fetch=True
            )
            if instance_info:
    ...
    def resolve_for_world(self, plugin_id: str, config_key: str, instance_id: int, world_name: str) -> Any:
    ...
    def resolve_for_rank(self, plugin_id: str, config_key: str, instance_id: int, rank_name: str) -> Any:
    ...
    def resolve_for_player(
        ...
        return self.get_effective_value(
            plugin_id, config_key, instance_id, world_name=world_name, rank_name=rank_name, player_uuid=player_uuid
        )
    ...
    def _get_server_config(self, plugin_id: str, config_key: str, server_name: str) -> Optional[Any]:
        ...
        try:
            result = self.db.execute_query(
                ...
                LIMIT 1
            """,
                (plugin_id, config_key, server_name),
                fetch=True,
            )
    ...
    def _get_world_config(self, plugin_id: str, config_key: str, instance_id: int, world_name: str) -> Optional[Any]:
        ...
        try:
            result = self.db.execute_query(
                ...
                LIMIT 1
            """,
                (plugin_id, config_key, instance_id, world_name),
                fetch=True,
            )
    ...
    def _get_rank_config(self, plugin_id: str, config_key: str, instance_id: int, rank_name: str) -> Optional[Any]:
        ...
        try:
            result = self.db.execute_query(
                ...
                LIMIT 1
            """,
                (plugin_id, config_key, instance_id, rank_name),
                fetch=True,
            )
    ...
    def _get_player_config(self, plugin_id: str, config_key: str, instance_id: int, player_uuid: str) -> Optional[Any]:
        ...
        try:
            result = self.db.execute_query(
                ...
                LIMIT 1
            """,
                (plugin_id, config_key, instance_id, player_uuid),
                fetch=True,
            )
